...
try:
    from colorama import Fore, Style, init  # Colored output (optional in server context)
...
class ApiPtTargetUnreachable(Exception):
    """Raised when the target cannot be reached (used by Django runner)."""
...
def test_connectivity():
    ...
    if r is None:
        ...
        raise ApiPtTargetUnreachable("Target unreachable — check URL, network, and TLS settings.")
...
def run_api_pt_scan(
    ...
    """
    Run all API penetration tests and return structured results (for MongoDB / API).
    Mutates module globals TARGET_URL, AUTH_TOKEN, etc. to match existing test_* functions.
    """
    global TARGET_URL, AUTH_TOKEN, USERNAME, PASSWORD, DELAY

    TARGET_URL = (target_url or "").strip()
    if not TARGET_URL or "your-api-url" in TARGET_URL.lower():
        raise ValueError("A valid target_url is required")

    AUTH_TOKEN = token if token is not None else os.environ.get("API_PT_TOKEN") or None
    USERNAME = (
        username
        if username is not None
        else os.environ.get("API_PT_USERNAME", "admin")
    )
    PASSWORD = (
        password
        if password is not None
        else os.environ.get("API_PT_PASSWORD", "password")
    )
    DELAY = (
        float(delay)
        if delay is not None
        else float(os.environ.get("API_PT_DELAY", "0.3"))
    )
...
def main():
  ...
  is illegal in most countries!
    ...
    try:
        run_api_pt_scan(TARGET_URL, token=AUTH_TOKEN, username=USERNAME, password=PASSWORD, delay=DELAY)
    except ApiPtTargetUnreachable:
        print(Fore.RED + "  ❌ Target unreachable! Check URL and try again.")
        sys.exit(1)
